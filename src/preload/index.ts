...
const api = {
  ...
  getGitBranches: () =>
    ipcRenderer.invoke('app:getGitBranches') as Promise<import('@/types/api').GitBranchesResult>,
  gitCheckout: (branch: string) =>
    ipcRenderer.invoke('app:gitCheckout', branch) as Promise<import('@/types/api').GitOpResult>,
  gitCreateBranch: (newName: string, fromBranch: string) =>
    ipcRenderer.invoke('app:gitCreateBranch', newName, fromBranch) as Promise<
      import('@/types/api').GitOpResult
    >,
  gitPull: () => ipcRenderer.invoke('app:gitPull') as Promise<import('@/types/api').GitOpResult>,
  getGitUncommittedChanges: () =>
    ipcRenderer.invoke('app:getGitUncommittedChanges') as Promise<
  ...
  getConfigValue: (key: string) =>
    ipcRenderer.invoke('app:getConfigValue', key) as Promise<string | null>,
  setConfigValue: (key: string, value: string) =>
    ipcRenderer.invoke('app:setConfigValue', key, value) as Promise<void>,
  ...
  saveNRAlertsForStack: (stack: string, alerts: NrAlert[]) =>
    ipcRenderer.invoke(
      ...
      stack,
      alerts
    ) as Promise<SaveNRAlertsForStackResult>,
