...
function ThresholdBadge({ alert }: { alert: NrAlert }) {
  ...
  if (type === 'print_duration') {
    return <Badge variant="secondary">{value}s</Badge>
  }
  if (type === 'error_rate') {
...
export function AlertListRow({ alert, originalIndex, onSelect }: AlertListRowProps) {
  ...
  return (
    <Item variant={'outline'} onClick={() => onSelect(originalIndex)} className="cursor-pointer">
      <ItemMedia>
        {!alert.enabled ? (
          <LucideCircle className="text-gray-500" size={16} strokeWidth={3} />
        ) : (
          <LucideCircleCheck className="text-green-500" size={16} strokeWidth={3} />
        )}
      </ItemMedia>
      <ItemContent>
        <ItemTitle>
          ...
          {alertType === 'print_duration' || alertType === 'error_rate' ? (
            <Badge
              variant={'outline'}
              className={`${alertType === 'print_duration' ? 'bg-blue-100 text-blue-500' : 'bg-orange-100 text-orange-800'}`}
            >
          ...
          {alert.close_violations_on_expiration && <Badge variant={'outline'}>LOS</Badge>}
        </ItemTitle>
