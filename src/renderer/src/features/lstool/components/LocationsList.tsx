...
import { LucideCopy, LucideHash, LucideHome, LucideMapPin, LucideTag } from 'lucide-react'
      ...
      <div className="mt-2 flex-1 min-h-0 overflow-auto space-y-2">
        {locations.map((loc) => {
          ...
          const openingLink = Array.isArray(loc.links)
            ? loc.links.find((l: any) => l.rel === 'openingTimesRules')
            : null
          const openingData = openingTimes[locId]
          const openingLoading = openingTimesLoading[locId]
          const openingErr = openingTimesError[locId]

          return (
            <div
              key={locId}
              className="rounded border bg-white/5 p-3 text-sm space-y-2 cursor-pointer hover:bg-white/10 transition-colors relative"
